...
INSTALL_ZSH_CUSTOMIZATIONS=${8:-"true"}
ZSH_ALIEN_THEME=${9:-"soft"}
...
if [ "${INSTALL_ZSH}" = "true" ]; then
    ...
    sed -i -e 's/^plugins=(git)$/plugins=(git zsh-autosuggestions zsh-syntax-highlighting)/g' ${user_rc_path}/.zshrc

    # Only applies to Alien theme
    git clone https://github.com/eendroroy/alien.git ${oh_my_install_dir}/custom/themes/alien
    cd ${oh_my_install_dir}/custom/themes/alien
    git submodule update --init --recursive

    nl=$'\n'

    sed -i -e $"\$a${nl}" ${user_rc_path}/.zshrc
    sed -i -e '$a# Alien Theme' ${user_rc_path}/.zshrc
    sed -i -e $"\$aexport ALIEN_THEME='${ZSH_ALIEN_THEME}'" ${user_rc_path}/.zshrc
    sed -i -e $"\$aexport ALIEN_VERSIONS_PROMPT='NODE'" ${user_rc_path}/.zshrc
    sed -i -e '$aexport ALIEN_SECTIONS_LEFT=(exit user path newline ssh venv prompt)' ${user_rc_path}/.zshrc
    sed -i -e '$aexport ALIEN_SECTIONS_RIGHT=(vcs_branch:async vcs_status:async vcs_dirty:async versions time battery)' ${user_rc_path}/.zshrc
    sed -i -e '$asource $ZSH/custom/themes/alien/alien.zsh' ${user_rc_path}/.zshrc

    # Change the default editor in Git
